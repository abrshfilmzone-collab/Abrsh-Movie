...
# --- 3. DATABASE SETUP ---
def init_db():
    conn = sqlite3.connect('abrsh_final_storage.db', check_same_thread=False)
    c = conn.cursor()
...
# --- 5. START HANDLER (WITH PHOTO & CORRECT TEXT) ---
...
def start(message):
    ...
    # ፎቶውን እንዲልክ (File ID ወይም URL መጠቀም ትችላለህ)
    photo_url = "https://telegra.ph/file/0c9769f36f6d4d1a6c8a2.jpg" # ካልሰራ አዲስ ሊንክ ተካበት
    try:
        bot.send_photo(message.chat.id, photo_url, caption=welcome_text, reply_markup=main_markup(message.from_user.id))
    except:
        bot.send_message(message.chat.id, welcome_text, reply_markup=main_markup(message.from_user.id))

# --- 🎬 ፊልም ልይ (CORRECT SEARCH LOGIC) ---
...
def perform_search(message):
    ...
    for r in results:
        ...
        markup.add(types.InlineKeyboardButton(btn_text, callback_data=f"buy_{r[0]}"))
...
def process_upload(message):
    if not message.document and not message.video:
        bot.send_message(ADMIN_ID, "❌ ስህተት፦ እባክዎ ቪዲዮ ወይም ፋይል ይላኩ!")
        return
    ...
    bot.send_message(ADMIN_ID, f"🎬 ስም፦ {f_name}\nአይነትና ዋጋ ይምረጡ፦", reply_markup=markup)
